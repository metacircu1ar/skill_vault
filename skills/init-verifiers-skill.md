...
**What it does.** Writes reusable verifier skills for your project. It scans the repo,
works out what kind of app each area is, asks you the details it can't infer (dev server
command, ready signal, login flow, test credentials), and generates `SKILL.md` files into
`.claude/skills/`. A Verify agent then picks them up automatically and can check that a
change actually works — not just that it compiles.

**How to use it.** Run `/init-verifiers` once per project. Answer the questions. Run it
again later to add verifiers for areas you skipped. The generated files are yours to edit.

**When to reach for it.** Once your project exposes something a machine can exercise: a
running UI, a CLI, or HTTP endpoints. It produces exactly three kinds — Playwright for web
UI, Tmux for CLI, HTTP for APIs.

**One thing to know.** It explicitly **refuses to generate verifiers for unit tests or
typechecking**, on the grounds that your build already covers those. So a project whose
verification story is schema, migrations, and a test suite gets nothing from it — wait
until there are real endpoints or screens. Discovery is by *folder name*: the directory
must contain `verifier` or the Verify agent won't find it.

---

> Description: *Create verifier skill(s) for automated verification of code changes.*
> Progress message: *analyzing your project and creating verifier skills.*
>
> This is a **generator**: it doesn't verify anything itself, it writes `SKILL.md` files
> into `.claude/skills/` that a Verify agent later discovers and runs. Discovery is by
> folder name — the name must contain `verifier` (case-insensitive).
...
- **The scope exclusion is the most important line.** "Do NOT create verifiers for unit
  tests or typechecking" means this tool has nothing to offer a project whose verification
  story is schema, migrations, and a test suite. It only produces the three functional
  kinds: Playwright, Tmux, HTTP.
- **Discovery is by substring, not frontmatter.** A verifier skill is found because its
  *folder* name contains `verifier`. Renaming the folder silently unregisters it.
- **The generated skill is instructed to edit itself.** The Self-Update section tells the
  verifier to distinguish "the feature is broken" from "my own setup instructions drifted",
  and to patch its own `SKILL.md` in the second case — after confirming with the user.
- The generated skill's first line — *"You are a verification executor. You receive a
  verification plan and execute it EXACTLY as written."* — makes clear these are meant to be
  driven by a separate planning step, not to improvise.
